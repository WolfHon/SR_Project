...
void Engine::CCollisionMgr::SetTerrainVtx(const VTXTEX* pTerrainVtx)
{
	MAPCOLLISION::iterator	iter = m_mapCollision.find(COLLISON_TERRAIN);
	if(iter == m_mapCollision.end())
		return ;

	((CTerrainCol*)iter->second)->SetTerrainVtx(pTerrainVtx);
}
