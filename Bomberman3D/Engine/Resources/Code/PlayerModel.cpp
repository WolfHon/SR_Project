...
{	
	...
	D3DXMatrixTranslation(&m_matBodyTrans
		, m_vBodyPos.x + m_vBodyPosPibotPos.x
		, m_vBodyPos.y + m_vBodyPosPibotPos.y
		, m_vBodyPos.z + m_vBodyPosPibotPos.z);
	...
	D3DXMatrixTranslation(&m_matBodyPibotTrans,		 -m_vBodyPosPibotPos.x,	  -m_vBodyPosPibotPos.y,	-m_vBodyPosPibotPos.z);
...
void Engine::CPlayerModel::Render(void)
{
	...
	m_pDevice->SetTransform(D3DTS_WORLD, &(m_matScale * m_matBodyPibotTrans * m_matBodyWorld * matWorld));
	...
	m_pDevice->SetTransform(D3DTS_WORLD,&(m_matScale * m_matHeadPibotTrans * m_matHeadWorld * m_matBodyWorld * matWorld));
	...
	m_pDevice->SetTransform(D3DTS_WORLD,&(m_matScale * m_matLeftArmPibotTrans * m_matLeftArmWorld * m_matBodyWorld * matWorld));
	...
	D3DXMatrixScaling(&m_matScale, m_vArmScale.x ,m_vArmScale.y ,m_vArmScale.z);
	m_pDevice->SetTransform(D3DTS_WORLD,&(m_matScale * m_matRightArmPibotTrans * m_matRightArmWorld * m_matBodyWorld * matWorld));
	...
	m_pDevice->SetTransform(D3DTS_WORLD,&(m_matScale * m_matLeftFootPibotTrans * m_matLeftFootWorld * m_matBodyWorld * matWorld));
	...
	D3DXMatrixScaling(&m_matScale, m_vFootScale.x ,m_vFootScale.y ,m_vFootScale.z);
	m_pDevice->SetTransform(D3DTS_WORLD,&(m_matScale * m_matRightFootPibotTrans * m_matRightFootWorld * m_matBodyWorld * matWorld));
...
void Engine::CPlayerModel::Animation(void)
{
	...
	m_matBodyWorld = m_matBodyRotationX * m_matBodyRotationY * m_matBodyRotationZ * m_matBodyTrans;
...
bool Engine::CPlayerModel::CompareHeadAngle(void)
{
	...
	//Compare X
	if(m_pNextFrame.HeadAngle.AngleX + 1 > m_pFrameAngle.HeadAngle.AngleX 
		&& m_pFrameAngle.HeadAngle.AngleX > m_pNextFrame.HeadAngle.AngleX - 1)
	{
		m_pFrameAngle.HeadAngle.AngleX = m_pNextFrame.HeadAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.HeadAngle.AngleX > m_pNextFrame.HeadAngle.AngleX + 1)
		(m_pFrameAngle.HeadAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.HeadAngle.AngleX < m_pNextFrame.HeadAngle.AngleX - 1)
		(m_pFrameAngle.HeadAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.HeadAngle.AngleY + 1 > m_pFrameAngle.HeadAngle.AngleY 
		&& m_pFrameAngle.HeadAngle.AngleY > m_pNextFrame.HeadAngle.AngleY - 1)
	{
		m_pFrameAngle.HeadAngle.AngleY = m_pNextFrame.HeadAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.HeadAngle.AngleY > m_pNextFrame.HeadAngle.AngleY + 1)
		(m_pFrameAngle.HeadAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.HeadAngle.AngleY < m_pNextFrame.HeadAngle.AngleY - 1)
		(m_pFrameAngle.HeadAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.HeadAngle.AngleZ + 1 > m_pFrameAngle.HeadAngle.AngleZ && 
		m_pFrameAngle.HeadAngle.AngleZ > m_pNextFrame.HeadAngle.AngleZ - 1)
	{
		m_pFrameAngle.HeadAngle.AngleZ = m_pNextFrame.HeadAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.HeadAngle.AngleZ > m_pNextFrame.HeadAngle.AngleZ + 1)
		(m_pFrameAngle.HeadAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
...
bool Engine::CPlayerModel::CompareLeftArmAngle(void)
{
	...
	//Compare X
	if(m_pNextFrame.LeftArmAngle.AngleX + 1 > m_pFrameAngle.LeftArmAngle.AngleX 
		&& m_pFrameAngle.LeftArmAngle.AngleX  > m_pNextFrame.LeftArmAngle.AngleX - 1)
	{
		m_pFrameAngle.LeftArmAngle.AngleX = m_pNextFrame.LeftArmAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.LeftArmAngle.AngleX > m_pNextFrame.LeftArmAngle.AngleX + 1)
		(m_pFrameAngle.LeftArmAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.LeftArmAngle.AngleX < m_pNextFrame.LeftArmAngle.AngleX - 1)
		(m_pFrameAngle.LeftArmAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.LeftArmAngle.AngleY + 1 > m_pFrameAngle.LeftArmAngle.AngleY
		&& m_pFrameAngle.LeftArmAngle.AngleY > m_pNextFrame.LeftArmAngle.AngleY - 1)
	{
		m_pFrameAngle.LeftArmAngle.AngleY = m_pNextFrame.LeftArmAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.LeftArmAngle.AngleY > m_pNextFrame.LeftArmAngle.AngleY + 1)
		(m_pFrameAngle.LeftArmAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.LeftArmAngle.AngleY < m_pNextFrame.LeftArmAngle.AngleY - 1)
		(m_pFrameAngle.LeftArmAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.LeftArmAngle.AngleZ + 1 > m_pFrameAngle.LeftArmAngle.AngleZ 
		&& m_pFrameAngle.LeftArmAngle.AngleZ > m_pNextFrame.LeftArmAngle.AngleZ - 1)
	{
		m_pFrameAngle.LeftArmAngle.AngleZ = m_pNextFrame.LeftArmAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.LeftArmAngle.AngleZ > m_pNextFrame.LeftArmAngle.AngleZ + 1)
		(m_pFrameAngle.LeftArmAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
...
bool Engine::CPlayerModel::CompareRightArmAngle(void)
{
	...
	//Compare X
	if((m_pNextFrame.RightArmAngle.AngleX + 1.f) >= m_pFrameAngle.RightArmAngle.AngleX 
		&& m_pFrameAngle.RightArmAngle.AngleX >= (m_pNextFrame.RightArmAngle.AngleX - 1.f))
	{
		m_pFrameAngle.RightArmAngle.AngleX = m_pNextFrame.RightArmAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.RightArmAngle.AngleX > m_pNextFrame.RightArmAngle.AngleX + 1)
		(m_pFrameAngle.RightArmAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.RightArmAngle.AngleX < m_pNextFrame.RightArmAngle.AngleX - 1)
		(m_pFrameAngle.RightArmAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.RightArmAngle.AngleY + 1 > m_pFrameAngle.RightArmAngle.AngleY 
		&& m_pFrameAngle.RightArmAngle.AngleY > m_pNextFrame.RightArmAngle.AngleY - 1)
	{
		m_pFrameAngle.RightArmAngle.AngleY = m_pNextFrame.RightArmAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.RightArmAngle.AngleY > m_pNextFrame.RightArmAngle.AngleY + 1)
		(m_pFrameAngle.RightArmAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.RightArmAngle.AngleY < m_pNextFrame.RightArmAngle.AngleY - 1)
		(m_pFrameAngle.RightArmAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.RightArmAngle.AngleZ + 1 > m_pFrameAngle.RightArmAngle.AngleZ 
		&& m_pFrameAngle.RightArmAngle.AngleZ > m_pNextFrame.RightArmAngle.AngleZ - 1)
	{
		m_pFrameAngle.RightArmAngle.AngleZ = m_pNextFrame.RightArmAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.RightArmAngle.AngleZ > m_pNextFrame.RightArmAngle.AngleZ + 1)
		(m_pFrameAngle.RightArmAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
...
bool Engine::CPlayerModel::CompareLeftFootAngle(void)
{
	...
	//Compare X
	if(m_pNextFrame.LeftFootAngle.AngleX + 1 > m_pFrameAngle.LeftFootAngle.AngleX 
		&& m_pFrameAngle.LeftFootAngle.AngleX > m_pNextFrame.LeftFootAngle.AngleX - 1)
	{
		m_pFrameAngle.LeftFootAngle.AngleX = m_pNextFrame.LeftFootAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.LeftFootAngle.AngleX > m_pNextFrame.LeftFootAngle.AngleX + 1)
		(m_pFrameAngle.LeftFootAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.LeftFootAngle.AngleX < m_pNextFrame.LeftFootAngle.AngleX - 1)
		(m_pFrameAngle.LeftFootAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.LeftFootAngle.AngleY + 1 > m_pFrameAngle.LeftFootAngle.AngleY 
		&& m_pFrameAngle.LeftFootAngle.AngleY > m_pNextFrame.LeftFootAngle.AngleY - 1)
	{
		m_pFrameAngle.LeftFootAngle.AngleY = m_pNextFrame.LeftFootAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.LeftFootAngle.AngleY > m_pNextFrame.LeftFootAngle.AngleY + 1)
		(m_pFrameAngle.LeftFootAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.LeftFootAngle.AngleY < m_pNextFrame.LeftFootAngle.AngleY - 1)
		(m_pFrameAngle.LeftFootAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.LeftFootAngle.AngleZ + 1 > m_pFrameAngle.LeftFootAngle.AngleZ
		&& m_pFrameAngle.LeftFootAngle.AngleZ > m_pNextFrame.LeftFootAngle.AngleZ - 1)
	{
		m_pFrameAngle.LeftFootAngle.AngleZ = m_pNextFrame.LeftFootAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.LeftFootAngle.AngleZ > m_pNextFrame.LeftFootAngle.AngleZ + 1)
		(m_pFrameAngle.LeftFootAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
...
bool Engine::CPlayerModel::CompareRightFootAngle(void)
{
	...
	//Compare X
	if(m_pNextFrame.RightFootAngle.AngleX + 1 > m_pFrameAngle.RightFootAngle.AngleX 
		&& m_pFrameAngle.RightFootAngle.AngleX  > m_pNextFrame.RightFootAngle.AngleX - 1)
	{
		m_pFrameAngle.RightFootAngle.AngleX = m_pNextFrame.RightFootAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.RightFootAngle.AngleX > m_pNextFrame.RightFootAngle.AngleX + 1)
		(m_pFrameAngle.RightFootAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.RightFootAngle.AngleX < m_pNextFrame.RightFootAngle.AngleX - 1)
		(m_pFrameAngle.RightFootAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.RightFootAngle.AngleY + 1 > m_pFrameAngle.RightFootAngle.AngleY 
		&& m_pFrameAngle.RightFootAngle.AngleY > m_pNextFrame.RightFootAngle.AngleY - 1)
	{
		m_pFrameAngle.RightFootAngle.AngleY = m_pNextFrame.RightFootAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.RightFootAngle.AngleY > m_pNextFrame.RightFootAngle.AngleY + 1)
		(m_pFrameAngle.RightFootAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.RightFootAngle.AngleY < m_pNextFrame.RightFootAngle.AngleY - 1)
		(m_pFrameAngle.RightFootAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.RightFootAngle.AngleZ + 1 > m_pFrameAngle.RightFootAngle.AngleZ
		&& m_pFrameAngle.RightFootAngle.AngleZ > m_pNextFrame.RightFootAngle.AngleZ - 1)
	{
		m_pFrameAngle.RightFootAngle.AngleZ = m_pNextFrame.RightFootAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.RightFootAngle.AngleZ > m_pNextFrame.RightFootAngle.AngleZ + 1)
		(m_pFrameAngle.RightFootAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
...
bool Engine::CPlayerModel::CompareBodyAngle(void)
{
	...
	//Compare X
	if(m_pNextFrame.BodyAngle.AngleX + 1 > m_pFrameAngle.BodyAngle.AngleX 
		&& m_pFrameAngle.BodyAngle.AngleX > m_pNextFrame.BodyAngle.AngleX - 1)
	{
		m_pFrameAngle.BodyAngle.AngleX = m_pNextFrame.BodyAngle.AngleX;
		ComX = true;
	}

	else if(m_pFrameAngle.BodyAngle.AngleX > m_pNextFrame.BodyAngle.AngleX + 1)
		(m_pFrameAngle.BodyAngle.AngleX) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.BodyAngle.AngleX < m_pNextFrame.BodyAngle.AngleX - 1)
		(m_pFrameAngle.BodyAngle.AngleX) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Y
	if(m_pNextFrame.BodyAngle.AngleY + 1 > m_pFrameAngle.BodyAngle.AngleY 
		&& m_pFrameAngle.BodyAngle.AngleY > m_pNextFrame.BodyAngle.AngleY - 1)
	{
		m_pFrameAngle.BodyAngle.AngleY = m_pNextFrame.BodyAngle.AngleY;
		ComY = true;
	}

	else if(m_pFrameAngle.BodyAngle.AngleY > m_pNextFrame.BodyAngle.AngleY + 1)
		(m_pFrameAngle.BodyAngle.AngleY) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	else if(m_pFrameAngle.BodyAngle.AngleY < m_pNextFrame.BodyAngle.AngleY - 1)
		(m_pFrameAngle.BodyAngle.AngleY) += m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();

	//Compare Z
	if(m_pNextFrame.BodyAngle.AngleZ + 1 > m_pFrameAngle.BodyAngle.AngleZ 
		&& m_pFrameAngle.BodyAngle.AngleZ > m_pNextFrame.BodyAngle.AngleZ - 1)
	{
		m_pFrameAngle.BodyAngle.AngleZ = m_pNextFrame.BodyAngle.AngleZ;
		ComZ = true;
	}

	else if(m_pFrameAngle.BodyAngle.AngleZ > m_pNextFrame.BodyAngle.AngleZ + 1)
		(m_pFrameAngle.BodyAngle.AngleZ) -= m_pFrameAngle.fTime * Get_TimeMgr()->GetTime();
