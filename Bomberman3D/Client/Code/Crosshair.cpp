...
Engine::OBJECT_RESULT CCrosshair::Update( void )
{
...
CCrosshair* CCrosshair::Create( LPDIRECT3DDEVICE9 pDevice, D3DXVECTOR3 vPos)
{
	CCrosshair*	pGameObject = new CCrosshair(pDevice);
...
HRESULT CCrosshair::Initialize( D3DXVECTOR3 vPos)
{
...
HRESULT CCrosshair::AddComponent( void )
{
	...
	//Texture
	pComponent = Engine::Get_ResourceMgr()->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_Crosshair");
...
void CCrosshair::Release( void )
{

}

void CCrosshair::CalculateOrtho( void )
{
	D3DVIEWPORT9 vp;
	float fFar = 1.f;
	float fNear = 0.f;
