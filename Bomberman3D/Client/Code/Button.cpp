...
CButton::CButton( LPDIRECT3DDEVICE9 pDevice )
...
Engine::OBJECT_RESULT CButton::Update( void )
{
	...
	m_ptMouse.x = Engine::CMouseMgr::GetInstance()->GetMousePos().x;
	m_ptMouse.y = Engine::CMouseMgr::GetInstance()->GetMousePos().y;
...
CButton* CButton::Create( LPDIRECT3DDEVICE9 pDevice, D3DXVECTOR3 vPos)
{
	CButton*	pGameObject = new CButton(pDevice);
...
HRESULT CButton::Initialize( D3DXVECTOR3 vPos)
{
	FAILED_CHECK(AddComponent());

	m_pInfo->m_vPos = vPos;

	m_RcButtonRect.top = 477.f;
	m_RcButtonRect.bottom = 556.f;
	m_RcButtonRect.left = 467.f;
	m_RcButtonRect.right = 759.f;

	//m_RcButtonRect.bottom;
...
void CButton::CalculateOrtho( void )
{
	D3DVIEWPORT9 vp;
	float fFar = 1.f;
	float fNear = 0.f;
